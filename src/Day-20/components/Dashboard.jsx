            ...
            <div className='flex gap-2 mt-10'>
                ...
                <div className='w-3/4 border-1 border-black flex flex-col items-center bg-white'>
                    ...
                    <form onSubmit={(e) => e.preventDefault()} className='mb-15'>
                        ...
                        />
                        <button type='submit' className='bg-black text-white px-4 py-1'>Search</button>
                    </form>
                                    ...
                                    filteredProducts.map((product) => (
                                        <div key={product.id} className='bg-amber-200 rounded-lg py-3 px-6 w-[230px]'>
                                            <div className='border-1 border-amber-50 bg-amber-100 rounded-lg'>
                                                <img className='hover:scale-105 transition-all duration-300 hover:bg-pink-400 hover:rounded-lg ease-in-out' id={product.id} src={product.thumbnail} alt={product.title} />
                                            </div>
                                            <div className='my-1 flex justify-between'>
                                                <h2 className='flex flex-nowrap overflow-hidden text-ellipsis whitespace-nowrap font-bold text-sm'>{product.title}</h2>
                                                <p className='text-sm font-light bg-red-300 w-10 rounded-full text-center px-1'>{product.rating}</p>
                                            </div>
                                            <div>
                                                <p className='border-1 border-amber-600 rounded-xl text-center text-[16px] font-bold'>Description: </p>
                                                <p className='h-25 overflow-auto text-sm font-light text-gray-600 border-1 border-amber-600 rounded-lg mt-2 py-1 px-2'>{product.description}</p>
                                            </div>
                                            ...
                                            <div className='my-4 flex justify-between'>
                                                <button
                                                    ...
                                                    className='bg-sky-400 border-sky-800 rounded-lg py-1 px-2 border-2 font-bold'>Buy now</button>
                                                <button
                                                    onClick={handleCartProduct}
                                                    className='bg-sky-400 border-sky-800 rounded-lg py-1 px-2 border-2 font-bold'>Cart</button>
                                            </div>
                                        </div>
                                    ))
                                )
